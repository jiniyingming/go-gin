...
func (dbVal *DdgBzj) Update(ctx context.Context, key ...interface{}) error {
	...
	sqlstr := `UPDATE ` + tableName + ` SET ` +
		`price = ?, total_price = ?, updated = ?` +
		` WHERE factory_id = ?`

	// run query
	utils.GetTraceLog(ctx).Debug("DB", zap.String("SQL", fmt.Sprint(sqlstr, dbVal.Price, dbVal.TotalPrice, dbVal.Updated, dbVal.FactoryID)))
	if tx != nil {
		_, err = tx.Exec(sqlstr, dbVal.Price, dbVal.TotalPrice, dbVal.Updated, dbVal.FactoryID)
	} else {
		_, err = dbConn.Exec(sqlstr, dbVal.Price, dbVal.TotalPrice, dbVal.Updated, dbVal.FactoryID)
	}
	return err
}
...
type QueryReq struct {
	LeftPrice string  `json:"left_price" form:"left_price"`
	FactoryId int     `json:"factory_id" form:"factory_id"`
	Price     float64 `json:"price" form:"price"`
	Uid       uint64  `json:"uid" form:"uid"`
	Type      int64   `json:"type" form:"type"`
	RelatedId int64   `json:"related_id" form:"related_id"`
	Action    uint8   `json:"action" form:"action"`
}

func QueryByCondition(ctx context.Context, lp *QueryReq, key ...interface{}) ([]DdgBzj, error) {
	var err error
	var dbConn *sql.DB
	...
	dest = append(dest, lp.FactoryId)

	if lp.LeftPrice != "" {
		sp := strings.Split(lp.LeftPrice, ",")

		fmt.Println(sp)

		if len(sp) == 1 {
			sqlstr += ` AND price >= ?`
			dest = append(dest, sp[0])
		} else if len(sp) == 2 {

			if sp[0] == "" && sp[1] != "" {
				sqlstr += ` AND price <= ?`
				dest = append(dest, sp[1])
			} else if sp[1] == "" && sp[0] != "" {
				sqlstr += ` AND price >= ?`
				dest = append(dest, sp[0])
			} else if sp[0] != "" && sp[1] != "" {
	...
	utils.GetTraceLog(ctx).Debug("DB", zap.String("SQL", fmt.Sprint(sqlstr, dest)))

	var rows *sql.Rows

	if tx != nil {
		rows, err = tx.Query(sqlstr, dest...)
	} else {
		rows, err = dbConn.Query(sqlstr, dest...)
	}
	...
	ddgBzjModel := make([]DdgBzj, 0)

	for rows.Next() {
		var ddgItem DdgBzj
		rows.Scan(&ddgItem.FactoryID, &ddgItem.Price, &ddgItem.TotalPrice, &ddgItem.Created, &ddgItem.Updated)
		fmt.Println(ddgItem)
		ddgBzjModel = append(ddgBzjModel, ddgItem)
	}
	return ddgBzjModel, nil
}
