...
func GetBaseMessageRecTableName(key ...interface{}) (string, error) {
	tableName, err := components.M.GetTable(components.E.Opts.DBConfig.Name, "base_message_rec", key...)
	if err != nil {
		return "", err
	}
...
func (bmr *BaseMessageRec) Insert(ctx context.Context, key ...interface{}) error {
	...
	// run query
	utils.GetTraceLog(ctx).Debug("DB", zap.String("SQL", fmt.Sprint(sqlstr, bmr.Title, bmr.Platform, bmr.SourceType, bmr.Code, bmr.Channel, bmr.UserID, bmr.SendStatus, bmr.TemplateID, bmr.ResultJSON, bmr.ParamJSON, bmr.CreatedAt, bmr.UpdatedAt)))
	if err != nil {
		return err
	}
	if tx != nil {
		res, err = tx.Exec(sqlstr, bmr.Title, bmr.Platform, bmr.SourceType, bmr.Code, bmr.Channel, bmr.UserID, bmr.SendStatus, bmr.TemplateID, bmr.ResultJSON, bmr.ParamJSON, bmr.CreatedAt, bmr.UpdatedAt)
	} else {
		res, err = dbConn.Exec(sqlstr, bmr.Title, bmr.Platform, bmr.SourceType, bmr.Code, bmr.Channel, bmr.UserID, bmr.SendStatus, bmr.TemplateID, bmr.ResultJSON, bmr.ParamJSON, bmr.CreatedAt, bmr.UpdatedAt)
	}
