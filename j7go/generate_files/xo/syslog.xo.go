...
type Syslog struct {
	...
	Referer    sql.NullString `json:"referer"`    // referer
	Adddt      time.Time      `json:"adddt"`      // adddt
	Guestcode  sql.NullString `json:"guestcode"`  // guestcode
	...
	Iplocation sql.NullString `json:"iplocation"` // iplocation
...
func GetSyslogTableName(key ...interface{}) (string, error) {
	tableName, err := components.M.GetTable(components.E.Opts.DBConfig.Name, "syslog", key...)
	if err != nil {
		return "", err
	}
...
func (s *Syslog) Insert(ctx context.Context, key ...interface{}) error {
	...
	sqlstr := `INSERT INTO ` + tableName +
		` (` +
		`ipaddress, url, referer, adddt, guestcode, param, app, module, control, uid, cid, gid, vid, iplocation, useragent, sagent, addtime` +
		`) VALUES (` +
		`?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?` +
		`)`

	// run query
	utils.GetTraceLog(ctx).Debug("DB", zap.String("SQL", fmt.Sprint(sqlstr, s.Ipaddress, s.URL, s.Referer, s.Adddt, s.Guestcode, s.Param, s.App, s.Module, s.Control, s.UID, s.Cid, s.Gid, s.Vid, s.Iplocation, s.Useragent, s.Sagent, s.Addtime)))
	if err != nil {
		return err
	}
	if tx != nil {
		res, err = tx.Exec(sqlstr, s.Ipaddress, s.URL, s.Referer, s.Adddt, s.Guestcode, s.Param, s.App, s.Module, s.Control, s.UID, s.Cid, s.Gid, s.Vid, s.Iplocation, s.Useragent, s.Sagent, s.Addtime)
	} else {
		res, err = dbConn.Exec(sqlstr, s.Ipaddress, s.URL, s.Referer, s.Adddt, s.Guestcode, s.Param, s.App, s.Module, s.Control, s.UID, s.Cid, s.Gid, s.Vid, s.Iplocation, s.Useragent, s.Sagent, s.Addtime)
	}
...
func (s *Syslog) Update(ctx context.Context, key ...interface{}) error {
	...
	// run query
	utils.GetTraceLog(ctx).Debug("DB", zap.String("SQL", fmt.Sprint(sqlstr, s.Ipaddress, s.URL, s.Referer, s.Adddt, s.Guestcode, s.Param, s.App, s.Module, s.Control, s.UID, s.Cid, s.Gid, s.Vid, s.Iplocation, s.Useragent, s.Sagent, s.Addtime, s.ID)))
	if tx != nil {
		_, err = tx.Exec(sqlstr, s.Ipaddress, s.URL, s.Referer, s.Adddt, s.Guestcode, s.Param, s.App, s.Module, s.Control, s.UID, s.Cid, s.Gid, s.Vid, s.Iplocation, s.Useragent, s.Sagent, s.Addtime, s.ID)
	} else {
		_, err = dbConn.Exec(sqlstr, s.Ipaddress, s.URL, s.Referer, s.Adddt, s.Guestcode, s.Param, s.App, s.Module, s.Control, s.UID, s.Cid, s.Gid, s.Vid, s.Iplocation, s.Useragent, s.Sagent, s.Addtime, s.ID)
	}
	return err
}
...
func SyslogsByIpaddress(ctx context.Context, ipaddress sql.NullString, key ...interface{}) ([]*Syslog, error) {
	...
	// sql query
	sqlstr := `SELECT ` +
		`id, ipaddress, url, referer, adddt, guestcode, param, app, module, control, uid, cid, gid, vid, iplocation, useragent, sagent, addtime ` +
		`FROM ` + tableName +
		` WHERE ipaddress = ?`
	...
	for queryData.Next() {
		...
		// scan
		err = queryData.Scan(&s.ID, &s.Ipaddress, &s.URL, &s.Referer, &s.Adddt, &s.Guestcode, &s.Param, &s.App, &s.Module, &s.Control, &s.UID, &s.Cid, &s.Gid, &s.Vid, &s.Iplocation, &s.Useragent, &s.Sagent, &s.Addtime)
		if err != nil {
			return nil, err
		}
...
func SyslogByID(ctx context.Context, id uint, key ...interface{}) (*Syslog, error) {
	...
	if tx != nil {
		err = tx.QueryRow(sqlstr, id).Scan(&s.ID, &s.Ipaddress, &s.URL, &s.Referer, &s.Adddt, &s.Guestcode, &s.Param, &s.App, &s.Module, &s.Control, &s.UID, &s.Cid, &s.Gid, &s.Vid, &s.Iplocation, &s.Useragent, &s.Sagent, &s.Addtime)
		if err != nil {
			return nil, err
		}
	} else {
		err = dbConn.QueryRow(sqlstr, id).Scan(&s.ID, &s.Ipaddress, &s.URL, &s.Referer, &s.Adddt, &s.Guestcode, &s.Param, &s.App, &s.Module, &s.Control, &s.UID, &s.Cid, &s.Gid, &s.Vid, &s.Iplocation, &s.Useragent, &s.Sagent, &s.Addtime)
		if err != nil {
			return nil, err
		}
	}
