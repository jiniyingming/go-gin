...
func GetDdgAdminFileTableName(key ...interface{}) (string, error) {
	tableName, err := components.M.GetTable(components.E.Opts.DBConfig.Name, "ddg_admin_files", key...)
	if err != nil {
		return "", err
	}
...
// DdgAdminFilesBySha retrieves a row from 'aypcddg.ddg_admin_files' as a DdgAdminFile.
//
// Generated from index 'sha'.
func DdgAdminFilesBySha(ctx context.Context, sha string, key ...interface{}) ([]*DdgAdminFile, error) {
	var err error
	var dbConn *sql.DB
	...
	// run query
	utils.GetTraceLog(ctx).Debug("DB", zap.String("SQL", fmt.Sprint(sqlstr, sha)))
	...
	if tx != nil {
		queryData, err = tx.Query(sqlstr, sha)
		if err != nil {
			return nil, err
		}
	} else {
		queryData, err = dbConn.Query(sqlstr, sha)
		if err != nil {
			return nil, err
		}
	}
	...
	for queryData.Next() {
		...
		// scan
		err = queryData.Scan(&daf.FileID, &daf.AdminID, &daf.FileType, &daf.FileMime, &daf.FileExt, &daf.FileName, &daf.Fileserver, &daf.Fpath, &daf.Storage, &daf.Sha, &daf.OssLogID, &daf.Width, &daf.Height, &daf.LastUploadTime)
		if err != nil {
			return nil, err
		}
