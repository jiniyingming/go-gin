...
func getMongoClient() (*components.MongoClient, *mongo.Collection, error) {
	...
	if err == nil {
		...
		if Opts.IsSplit == 1 {
			collection = fmt.Sprintf("%s-%d-%d-%d", collection, now.Year(), now.Month(), now.Day())
		}
		mgc = mgcli.Client.Database(Opts.Mongodb).Collection(collection)
		return mgcli, mgc, nil
	}
	return nil, nil, err
}
