...
func (s *server) Start() error {
	...
	if s.opts.EnableACME {
		log.Logf("Listening on s.opts.EnableACME")
		// should we check the address to make sure its using :443?
		l = autocert.NewListener(s.opts.ACMEHosts...)
	} else if s.opts.EnableTLS && s.opts.TLSConfig != nil {
		log.Logf("Listening on s.opts.EnableTLS s.opts.TLSConfig")
		l, err = tls.Listen("tcp", s.address, s.opts.TLSConfig)
	} else {
		// otherwise plain listen
		log.Logf("Listening on s.opts.EnableACME // otherwise plain listen")
	...
	log.Logf("Listening on @l.Addr() : %s", l.Addr().String())
	log.Logf("Listening on @s.address : %s", s.address)
